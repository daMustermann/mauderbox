                    ...
                    if let Some(pid_str) = line.split_whitespace().last() {
                        if let Ok(pid) = pid_str.parse::<u32>() {
                                ...
                                .output()
                            {
                                ...
                                let tasklist_lower = tasklist_str.to_lowercase();
                                if tasklist_lower.contains("voicebox") || tasklist_lower.contains("python") {
...
async fn stop_server(state: State<'_, ServerState>) -> Result<(), String> {
    ...
    if let Some(pid) = pid {
        ...
        #[cfg(windows)]
        {
            ...
            // NOTE: cleanup_orphan_processes() is now only called on app exit (RunEvent::Exit)
            // to avoid killing the server we just started
        }
    ...
    tauri::Builder::default()
        ...
        .on_window_event(|window, event| {
            if let WindowEvent::CloseRequested { api, .. } = event {
                ...
                // Kill server processes immediately on close
                #[cfg(windows)]
                {
                    println!("Window close requested - cleaning up processes...");
                    
                    // Kill voicebox-server.exe
                    let _ = std::process::Command::new("taskkill")
                        .args(["/IM", "voicebox-server.exe", "/F"])
                        .creation_flags(0x08000000) // CREATE_NO_WINDOW
                        .output();
                    
                    // Kill python processes running backend.main
                    let _ = std::process::Command::new("powershell")
                        .args(["-NoProfile", "-WindowStyle", "Hidden", "-Command", 
                            "Get-CimInstance Win32_Process | Where-Object { $_.Name -eq 'python.exe' -and $_.CommandLine -like '*backend.main*' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }"])
                        .creation_flags(0x08000000) // CREATE_NO_WINDOW
                        .output();
                    
                    println!("Cleanup completed, closing window...");
                }

                // Emit event to frontend to check setting
                if let Err(e) = app_handle.emit("window-close-requested", ()) {
                    eprintln!("Failed to emit window-close-requested event: {}", e);
                }

                // Close window after a short delay to allow frontend to save state
                let window_for_close = window_clone;
                tokio::spawn(async move {
        ...
        .run(|app, event| {
            match &event {
                RunEvent::Exit => {
                    ...
                    if !keep_running {
                        ...
                        if let Some(pid) = pid {
                            ...
                            #[cfg(windows)]
                            {
                                ...
                                // Layer 5: Aggressive cleanup of orphans
                    ...
                    // Always run cleanup on exit regardless of keep_running setting
                    // This catches any orphaned processes from previous runs
                    #[cfg(windows)]
                    {
                        println!("Running final orphan cleanup...");
                        cleanup_orphan_processes();
                    }
...
fn cleanup_orphan_processes() {
    println!("Scanning for orphan Voicebox processes...");
    
    // Kill voicebox-server.exe processes
    let _ = std::process::Command::new("taskkill")
        .args(["/IM", "voicebox-server.exe", "/F"])
        .output();
    
    // PowerShell command to find and kill python.exe running backend.main
    let ps_script = "Get-CimInstance Win32_Process | Where-Object { $_.Name -eq 'python.exe' -and $_.CommandLine -like '*backend.main*' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }";
    
    match std::process::Command::new("powershell")
        .args(["-NoProfile", "-Command", ps_script])
        .output() 
    {
        Ok(_) => println!("Orphan cleanup command executed."),
        Err(e) => eprintln!("Failed to execute orphan cleanup: {}", e),
    }
}

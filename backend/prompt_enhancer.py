...
def enhance_prompt(
    ...
    # Stronger constraint added: do NOT introduce new subjects or change the topic.
    # If `preserve_topic` is True, the model will be instructed to stay strictly on-topic.
    if lang_lower in ["de", "german", "deutsch"]:
        system_prompt = (
            ...
            "WICHTIG: Führe keine neuen Themen ein und weiche nicht vom ursprünglichen Thema ab. "
            "Wenn der Originaltext z.B. vom Kochen handelt, darfst du nur beim Kochen bleiben."
        )
        ...
        # Default to English
        topic_constraint = " Do NOT introduce new topics or change the subject of the original text. " if preserve_topic else ""
        system_prompt = (
            ...
            "to make the scene come alive. Respond only with the enhanced text in English." + topic_constraint
        )
        instruction = f"{system_prompt}\n\nOriginal text: {text}\n\nEnhanced text:"
    ...
    # Helper: simple tokenization for overlap check
    def _words(s: str):
        import re
        words = re.findall(r"\b\w+\b", s.lower())
        stopwords = {
            'the','and','a','an','of','to','in','on','mit','und','der','die','das','ein','eine',
            'de','la','le','el','les','los','las','un','une','y','et','zu','von'
        }
        return [w for w in words if w not in stopwords]

    def _overlap_ratio(orig: str, enhanced_text: str) -> float:
        o = set(_words(orig))
        e = set(_words(enhanced_text))
        if not o:
            return 1.0
        return len(o & e) / len(o)

    # Generate with up to 2 retry attempts if topic preservation fails
    max_attempts = 2 if preserve_topic else 1
    attempt = 0
    enhanced = ""
    while attempt < max_attempts:
        ...
        # Extract only the generated part (same as before)
        ...
        # Quick topic-preservation check
        if preserve_topic:
            overlap = _overlap_ratio(text, enhanced)
            # If overlap is low, retry with stricter instruction if we have another attempt
            if overlap < 0.25 and attempt < max_attempts:
                # Append explicit strict constraint and regenerate with low temperature
                strict_addendum = (
                    "\n\nSTRICT INSTRUCTION: Do not introduce new topics or entities. "
                    "Keep the same subject and reuse the key nouns from the original text."
                )
                instruction = instruction + strict_addendum
                inputs = _tokenizer(
                    instruction,
                    return_tensors="pt",
                    truncation=True,
                ...
                # reduce creativity for retry
                temperature = min(0.1, temperature)
                continue
            # If still bad after retries, fall back to original text
            if overlap < 0.15:
                print(f"[prompt_enhancer] topic-preservation failed (overlap={overlap:.2f}), returning original text.")
                return text
        break

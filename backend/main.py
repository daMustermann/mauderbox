...
import uvicorn
import argparse
# torch is only imported when needed (not in frozen binary)
try:
    import torch
...
# prompt_enhancer is imported lazily in endpoints that need it (not available in binary mode)
...
async def health():
    ...
    # Check for GPU availability (CUDA/ROCm or MPS)
    has_cuda = False
    has_mps = False
    ...
    if has_cuda and torch is not None:
        ...
        if hip_version:
            gpu_type = f"ROCm (HIP {hip_version})"
        else:
...
@app.get("/prompt-enhancer/status")
async def prompt_enhancer_status():
    """Check if prompt enhancer is loaded and downloaded."""
    try:
        from . import prompt_enhancer
    except (ImportError, ModuleNotFoundError):
        raise HTTPException(status_code=503, detail="Prompt enhancer not available (torch not loaded)")
    
    return {
        "loaded": prompt_enhancer.is_loaded(),
        "downloaded": prompt_enhancer.is_model_downloaded(),
        "model_id": prompt_enhancer.MODEL_ID,
    }


@app.post("/prompt-enhancer/load")
async def load_prompt_enhancer():
    """Load the prompt enhancement model with download tracking."""
    try:
        from . import prompt_enhancer
    except (ImportError, ModuleNotFoundError):
        raise HTTPException(status_code=503, detail="Prompt enhancer not available (torch not loaded)")
    
    if prompt_enhancer.is_loaded():
        return {"message": "Model already loaded", "loaded": True}
    
    task_manager = get_task_manager()
    model_name = "prompt-enhancer"
    
    # Check if model needs to be downloaded
    if not prompt_enhancer.is_model_downloaded():
        # Start download tracking
        task_manager.start_download(model_name)
        
        try:
            await prompt_enhancer.load_model_async()
            task_manager.complete_download(model_name)
            
            return {
                "message": "Prompt enhancer loaded successfully",
                "loaded": True,
            }
...
@app.post("/prompt-enhancer/unload")
async def unload_prompt_enhancer():
    """Unload the prompt enhancement model to free memory."""
    try:
        from . import prompt_enhancer
    except (ImportError, ModuleNotFoundError):
        raise HTTPException(status_code=503, detail="Prompt enhancer not available (torch not loaded)")
    
    prompt_enhancer.unload_model()
    return {"message": "Model unloaded", "loaded": False}


@app.post("/prompt-enhancer/enhance", response_model=models.PromptEnhanceResponse)
async def enhance_prompt(data: models.PromptEnhanceRequest):
    """Enhance a text prompt to be more descriptive (with auto-download)."""
    try:
        from . import prompt_enhancer
    except (ImportError, ModuleNotFoundError):
        raise HTTPException(status_code=503, detail="Prompt enhancer not available (torch not loaded)")
    ...
    # Auto-download and load if not ready
    if not prompt_enhancer.is_loaded():
        if not prompt_enhancer.is_model_downloaded():
            # Start download
            task_manager.start_download(model_name)
    ...
    try:
        ...
        return models.PromptEnhanceResponse(
            original=data.text,
            enhanced=enhanced,
        )
    except Exception as e:
...
async def tts_generate(
    ...
    """Generate speech and return audio as WAV (no history entry)."""
    task_manager = get_task_manager()
...
async def generate_speech(
    ...
    try:
        ...
        task_manager.update_generation_phase(generation_id, "generating")
        ...
        # Save audio
        task_manager.update_generation_phase(generation_id, "saving")
        ...
        task_manager.complete_generation(generation_id, actual_duration_seconds=generation_duration)
...
async def toggle_generation_favorite(
    ...
    result = await history.toggle_favorite(generation_id, db)
    if not result:
        raise HTTPException(status_code=404, detail="Generation not found")
    return result
...
async def export_generation(
    ...
    try:
        ...
        gen_text = str(generation.text)
        safe_text = "".join(c for c in gen_text[:30] if c.isalnum() or c in (' ', '-', '_')).strip()
        if not safe_text:
...
async def export_generation_audio(
    ...
    """Export only the audio file from a generation. Supports 'wav' or 'mp3'."""
    generation = db.query(DBGeneration).filter_by(id=generation_id).first()
    if not generation:
        raise HTTPException(status_code=404, detail="Generation not found")
    
    audio_path = Path(str(generation.audio_path))
    ...
    if format.lower() == "mp3":
        try:
            ...
            # Fallback to WAV if conversion fails, but keep requested filename extension to avoid confusion? 
            # No, correct fallback is to return WAV.
            pass
...
async def transcribe_audio(
    ...
    try:
        # Get audio duration
        from .utils.audio import load_audio, save_audio, trim_audio_to_duration
        audio, sr = load_audio(tmp_path)
        duration = len(audio) / sr
        max_transcribe_duration = 30.0
        if duration > max_transcribe_duration:
            audio = trim_audio_to_duration(audio, sr, max_duration=max_transcribe_duration)
            save_audio(audio, tmp_path, sr)
            duration = max_transcribe_duration
        ...
        try:
            text, words_dicts = await whisper_model.transcribe_with_confidence(tmp_path, language)

            # Convert dicts to models
            words = [
                models.TranscriptionWord(word=str(w['word']), confidence=float(w['confidence'])) 
...
async def get_model_status():
    ...
    def check_prompt_enhancer_loaded():
        """Check if prompt enhancer model is loaded."""
        try:
            from . import prompt_enhancer
            return prompt_enhancer.is_loaded()
        except Exception:
            return False
    ...
    model_configs = [
        ...
        {
            "model_name": "prompt-enhancer",
            "display_name": "Prompt Enhancer (LFM 1.2B)",
            "hf_repo_id": "huihui-ai/Huihui-LFM2.5-1.2B-Instruct-abliterated",
            "model_size": "1.2B",
            "check_loaded": check_prompt_enhancer_loaded,
        },
    ]
    ...
    for config in model_configs:
        try:
            ...
            # Method 1: Check if model can be loaded via hf_hub_download with local_files_only
            # This is the most reliable method as it uses the same logic as from_pretrained
            if not downloaded:
                try:
                    # Try to find config.json or model.safetensors
                    # checking availability of any critical file is enough to say it's "mostly" there
                    files_to_check = ["config.json", "model.safetensors", "pytorch_model.bin"]
                    ...
                    if found_any:
                        ...
                        # Try to get size from scan_cache_dir if available, otherwise we can't easily get it
                        # without crawling the obscure HF cache structure manually
                        if size_mb is None and cache_info:
                             repo_id = config["hf_repo_id"]
                             for repo in cache_info.repos:
            ...
            # Method 2: Fallback to scanning cache info (if available and Method 1 didn't set size or failed)
            if (not downloaded or size_mb is None) and cache_info:
                repo_id = config["hf_repo_id"]
                for repo in cache_info.repos:
                    if repo.repo_id == repo_id:
                        downloaded = True
...
async def trigger_model_download(request: models.ModelDownloadRequest):
    ...
    model_configs = {
        ...
        "prompt-enhancer": {
            "model_size": "1.2B",
            "load_func": lambda: _load_prompt_enhancer(),
        },
    }
    
    def _load_prompt_enhancer():
        """Lazy-load prompt enhancer module and call load_model()."""
        from . import prompt_enhancer
        return prompt_enhancer.load_model()
...
async def delete_model(model_name: str):
    ...
    try:
        ...
        elif config["model_type"] == "prompt_enhancer":
            try:
                from . import prompt_enhancer
                if prompt_enhancer.is_loaded():
                    prompt_enhancer.unload_model()
            except (ImportError, ModuleNotFoundError):
                pass  # Prompt enhancer not available, skip unload
...
async def get_active_tasks():
    ...
    for gen_task in task_manager.get_active_generations():
        ...
        elapsed = (datetime.utcnow() - gen_task.started_at).total_seconds()
        ...
        active_generations.append(models.ActiveGenerationTask(
            ...
            text_length=gen_task.text_length,
            started_at=gen_task.started_at,
            phase=gen_task.phase,
            estimated_duration_seconds=gen_task.estimated_duration_seconds,
...
def _get_gpu_status() -> str:
    ...
    if not TORCH_AVAILABLE or torch is None:
        return "None (CPU only - torch not available)"

...
**Form Data:**
- `file`: Audio file
- `language` (optional): Language hint (`zh`, `en`, `ja`, `ko`, `de`, `fr`, `ru`, `pt`, `es`, `it`)
...
voicebox uses whatever PyTorch build is installed. On AMD GPUs (Linux), a ROCm
build of PyTorch will be detected as CUDA-capable and used automatically.

**Optional device override:** set `VOICEBOX_DEVICE` to `cpu`, `cuda`, or `mps`.

**ROCm setup (Linux example):**

```bash
pip install --index-url https://download.pytorch.org/whl/rocm7.1.1 torch torchvision torchaudio
```

...
async def add_profile_sample(
    ...
    # Load and trim audio to max duration
    audio, sr = load_audio(audio_path)
    trimmed_audio = trim_audio_to_duration(audio, sr, max_duration=30.0)

    # Validate trimmed audio
    is_valid, error_msg = validate_reference_audio_data(trimmed_audio, sr)
    if not is_valid:
    ...
    # Save trimmed audio to profile directory
    dest_path = profile_dir / f"{sample_id}.wav"
    save_audio(trimmed_audio, str(dest_path), sr)
...
async def create_voice_prompt_for_profile(
    ...
    Args:
        ...
        model_size: Target model size (ensures correct prompt tensor size)
    ...
    # Ensure the correct model is loaded if a specific size is requested
    if model_size:

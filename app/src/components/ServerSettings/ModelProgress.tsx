...
export function ModelProgress({ modelName, displayName, enabled = true }: ModelProgressProps) {
  const [progress, setProgress] = useState<ModelProgressType | null>(null);
  const [isSubscribed, setIsSubscribed] = useState(false);
  const serverUrl = useServerStore((state) => state.serverUrl);

  useEffect(() => {
    if (!enabled || !serverUrl || isSubscribed) {
      return;
    }
  ...
  }, [enabled, serverUrl, modelName, isSubscribed]);

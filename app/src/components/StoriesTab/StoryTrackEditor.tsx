...
export function StoryTrackEditor({ storyId, items }: StoryTrackEditorProps) {
  ...
  const handleZoomFit = () => {
    ...
    const desired = containerWidth / (totalDurationMs / 1000);
    setPixelsPerSecond(Math.min(MAX_PIXELS_PER_SECOND, Math.max(MIN_PIXELS_PER_SECOND, desired)));
  };
  ...
  const handleTrimMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (trimSide === 'start') {
        ...
        if (snapToGrid) {
          newTrimStart = Math.round(newTrimStart / SNAP_INTERVAL_MS) * SNAP_INTERVAL_MS;
        }
      } else {
        ...
        if (snapToGrid) {
          newTrimEnd = Math.round(newTrimEnd / SNAP_INTERVAL_MS) * SNAP_INTERVAL_MS;
        }
      }
    ...
    [trimmingItem, trimSide, trimStartX, pixelsToMs, snapToGrid],
  );
  ...
  const handleDragEnd = useCallback(() => {
    ...
    const newTimeMs = snapToGrid
      ? Math.round(rawTimeMs / SNAP_INTERVAL_MS) * SNAP_INTERVAL_MS
      : rawTimeMs;
    ...
    setDraggingItem(null);
  }, [draggingItem, dragPosition, items, tracks, pixelsToMs, snapToGrid, storyId, moveItem, toast]);
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 border-t border-border/70 bg-background/95 backdrop-blur supports-backdrop-filter:bg-background/60 z-50">
      <div
        className="border-t border-border/60 bg-card/40 backdrop-blur-2xl overflow-hidden relative"
        ref={containerRef}
      >
        ...
        {/* Toolbar */}
        <div className="flex items-center justify-between px-3 py-2 border-b border-border/70 bg-card/60 mt-2">
          ...
          <div className="flex items-center gap-2">
            <Button
              variant="ghost"
              size="icon"
              className={cn('h-6 w-6', snapToGrid && 'bg-primary/10 text-primary')}
        ...
        <div className="flex" style={{ height: `${timelineContainerHeight}px` }}>
          ...
          <div className="w-16 shrink-0 border-r border-border/70 bg-card/60 overflow-hidden">
            {/* Spacer for time ruler */}
            <div className="h-6 border-b border-border/70 bg-muted/30" />
            {/* Track labels */}
            <div style={{ height: `${tracksAreaHeight}px` }}>
              {tracks.map((trackNumber, index) => (
                ...
                >
                  <span className="text-2.5 text-muted-foreground select-none">
                    {trackNumber}
                  </span>
                </div>
              ))}
          ...
          >
            ...
            <button
              type="button"
              className="h-6 border-b border-border/70 bg-muted/20 sticky top-0 z-10 cursor-pointer text-left"
            ...
            >
              {timeMarkers.map((ms) => (
                ...
                >
                  ...
                  <span className="text-2.5 text-muted-foreground ml-1 select-none">
                    {formatTime(ms)}
                  </span>
                </div>
              ))}
            ...
            >
              ...
              {tracks.map((trackNumber, index) => (
                <div
                  key={trackNumber}
                  className={cn(
                    'absolute left-0 right-0 border-b border-border/60 pointer-events-none',
                    index % 2 === 0 ? 'bg-card/30' : 'bg-muted/10',
                  )}
                  style={{
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="absolute top-0 left-1 right-1 z-10">
                        <p className="text-2.25 font-medium text-foreground/90 truncate">
                          {item.profile_name}
                        </p>
                      </div>
                      {/* Waveform */}
              ...
              <div
                className="absolute top-0 bottom-0 w-1 bg-primary z-30 pointer-events-none rounded-full"

...
export function AudioPlayer() {
  ...
  const useNativePlayback = useMemo(() => {
    ...
    const assignedChannels = (channels as AudioChannel[]).filter((ch) =>
      ...
      wavesurfer.on('ready', async () => {
        ...
        if (
          ...
          try {
            ...
            const assignedChannels = (runtimeChannels as AudioChannel[]).filter((ch) =>
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="sticky bottom-0 left-0 right-0 mx-auto w-[95%] max-w-5xl rounded-t-3xl border border-b-0 border-white/10 bg-black/80 backdrop-blur-2xl shadow-[0_-10px_50px_rgba(0,0,0,0.5)] z-50 p-2 mt-auto"
      >
      {/* Glow effect */}
      <div className="absolute inset-0 rounded-3xl bg-linear-to-r from-primary/10 via-transparent to-primary/10 pointer-events-none" />

      <div className="flex items-center gap-4 px-2">
          {/* Play/Pause Button - Prominent */}
          <div className="relative group">
            <div className="absolute inset-0 bg-primary/30 blur-md rounded-full group-hover:bg-primary/50 transition-colors duration-300" />
            <Button
              variant="default"
              size="icon"
              onClick={handlePlayPause}
              disabled={isLoading || duration === 0}
              className="relative h-14 w-14 rounded-full bg-primary hover:bg-primary/90 text-primary-foreground shadow-xl border border-white/20 shrink-0 z-10"
            >
          ...
          <div className="flex flex-col flex-1 min-w-0 py-2">
            ...
            <div className="flex items-center justify-between mb-1 px-1">
              <div className="text-sm font-medium truncate text-foreground/90 max-w-md">
                {title || 'Unknown Track'}
              </div>
              <div className="flex items-center gap-1 text-xs font-mono text-muted-foreground bg-black/20 px-2 py-0.5 rounded-full">
                <span className="text-primary">{formatAudioDuration(currentTime)}</span>
            ...
            {/* Middle: Waveform */}
            <div className="relative h-12 w-full rounded-lg overflow-hidden group/wave">
              <div ref={waveformRef} className="w-full h-full opacity-80 transition-opacity duration-300 group-hover/wave:opacity-100" />
              {/* Slider Overlay for dragging */}
              {duration > 0 && (
                 <div className="absolute inset-0 opacity-0 hover:opacity-100 transition-opacity duration-200">
                    <Slider
                      ...
                      max={100}
                      step={0.1}
                      className="w-full h-full cursor-pointer py-0"
                    />
                 </div>
              )}
            </div>
            ...
            {(isLoading || error) && (
              <div className="absolute top-0 right-0 left-0 bg-black/50 backdrop-blur flex items-center justify-center h-full rounded-2xl z-20">
                 {isLoading && <span className="text-xs text-primary animate-pulse">Loading audio data...</span>}
          ...
          {/* Right Controls */}
          <div className="flex items-center gap-2 pr-2 border-l border-white/5 pl-4 ml-2">
             {/* Loop */}
            <Button
              variant="ghost"
              size="icon"
              onClick={toggleLoop}
              className={cn("h-8 w-8 rounded-full transition-all", isLooping ? 'bg-primary/20 text-primary shadow-[0_0_10px_rgba(124,58,237,0.3)]' : 'text-muted-foreground hover:text-foreground')}
            >
            ...
            <Button
              ...
              size="icon"
              onClick={handleClose}
              className="h-8 w-8 rounded-full hover:bg-destructive/20 hover:text-destructive transition-colors ml-2"
            >
              <X className="h-4 w-4" />
            </Button>
          </div>

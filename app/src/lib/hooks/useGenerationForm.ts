...
export function useGenerationForm(options: UseGenerationFormOptions = {}) {
  ...
  const lastLanguage = useUIStore((state) => state.lastLanguage);
  const setLastLanguage = useUIStore((state) => state.setLastLanguage);
  const lastModelSize = useUIStore((state) => state.lastModelSize);
  const setLastModelSize = useUIStore((state) => state.setLastModelSize);
  ...
  const form = useForm<GenerationFormValues>({
    ...
    defaultValues: {
      text: '',
      language: (lastLanguage as LanguageCode) || 'en',
      seed: undefined,
      modelSize: (lastModelSize as '1.7B' | '0.6B') || '1.7B',
      instruct: '',
  ...
  useEffect(() => {
    if (watchedLanguage && watchedLanguage !== lastLanguage) {
      setLastLanguage(watchedLanguage);
    }
  }, [watchedLanguage, lastLanguage, setLastLanguage]);
  
  useEffect(() => {
    if (watchedModelSize && watchedModelSize !== lastModelSize) {
      setLastModelSize(watchedModelSize);
    }
  }, [watchedModelSize, lastModelSize, setLastModelSize]);
